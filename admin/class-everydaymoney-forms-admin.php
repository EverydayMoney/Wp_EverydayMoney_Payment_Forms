...
class EverydayMoney_Admin
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        add_action('admin_menu', 'em_application_tech_add_settings_page');
        add_action('admin_init', 'em_application_tech_register_setting_page');

        function em_application_tech_add_settings_page()
        {
            add_submenu_page('edit.php?post_type=everydaymoney_form', 'Settings', 'Settings', 'edit_posts', basename(__FILE__), 'em_application_tech_setting_page');
        }
        function em_application_tech_register_setting_page()
        {
            register_setting('em-t-everydaymoney-settings-group', 'emMode');
            register_setting('em-t-everydaymoney-settings-group', 'emWebhookUrl');
        ...
        function em_application_tech_setting_page()
        {
?>
            <div class="wrap">
                <h1>EverydayMoney Forms Settings</h1>

                <!-- <h4>Optional: To avoid situations where bad network makes it impossible to verify transactions, set your webhook URL <a href="https://dashboard.everydaymoney.app/#/settings/developer">here</a> to the URL below<strong style="color: red"><pre><code><?php echo admin_url("admin-ajax.php") . "?action=kkd_paystack_pff"; ?></code></pre></strong></h4> -->
                <h2>API Keys Settings</h2>
                <span>Get your API Keys <a href="https://everydaymoney.app" target="_blank">here</a> </span>
                <form method="post" action="options.php">
                    <?php settings_fields('em-t-everydaymoney-settings-group');
                    do_settings_sections('em-t-everydaymoney-settings-group'); ?>
                    <table class="form-table em_t_everydaymoney_setting_page">
                        <tr valign="top">
                            <th scope="row">Mode</th>

                            <td>
                                <select class="form-control" name="emMode" id="parent_id">
                                    <option value="test" <?php echo em_application_tech_txncheck('test', esc_attr(get_option('emMode'))) ?>>Test Mode</option>
                                    <option value="live" <?php echo em_application_tech_txncheck('live', esc_attr(get_option('emMode'))) ?>>Live Mode</option>
                                </select>
                            </td>
                        </tr>

                        <tr valign="top">
                            <th scope="row">Charges Inclusive of Amount</th>
                            <td>
                                <select class="form-control" name="emChargeInclusive" id="charge_mode">
                                    <option value="yes" <?php echo em_application_tech_txncheck('yes', esc_attr(get_option('emChargeInclusive'))) ?>>Yes</option>
                                    <option value="no" <?php echo em_application_tech_txncheck('no', esc_attr(get_option('emChargeInclusive'))) ?>>No</option>
                                </select>
                            </td>
                        </tr>

                        <tr valign="top">
                            <th scope="row">Your Secret Key</th>
                            <td><input type="text" name="emSecretKeyKey" value="<?php echo esc_attr(get_option('emSecretKeyKey')); ?>" /></td>
                        </tr>
                        <tr valign="top">
                            <th scope="row">Your Public Key</th>
                            <td><input type="text" name="emPublicKey" value="<?php echo esc_attr(get_option('emPublicKey')); ?>" /></td>
                        </tr>
                    ...
                    <table class="form-table em_t_everydaymoney_setting_page" id="paystack_setting_fees">
                        <h2>Control Settings</h2>

                        <tr valign="top">
                            <th scope="row">Webhook URL <br> <small>(Ensures Payement goes through outside this app)</small></th>
        ...
        add_action('init', 'register_em_application_tech');
        function register_em_application_tech()
        {
            $labels = array(
                'name' => _x('EverydayMoney Forms', 'everydaymoney_form'),
                'singular_name' => _x('EverydayMoney Form', 'everydaymoney_form'),
                'add_new' => _x('Add New', 'everydaymoney_form'),
                'add_new_item' => _x('Add EverydayMoney Form', 'everydaymoney_form'),
                'edit_item' => _x('Edit EverydayMoney Form', 'everydaymoney_form'),
                'new_item' => _x('EverydayMoney Form', 'everydaymoney_form'),
                'view_item' => _x('View EverydayMoney Form', 'everydaymoney_form'),
                'all_items' => _x('All Forms', 'everydaymoney_form'),
                'search_items' => _x('Search EverydayMoney Forms', 'everydaymoney_form'),
                'not_found' => _x('No EverydayMoney Forms found', 'everydaymoney_form'),
                'not_found_in_trash' => _x('No EverydayMoney Forms found in Trash', 'everydaymoney_form'),
                'parent_item_colon' => _x('Parent EverydayMoney Form:', 'everydaymoney_form'),
                'menu_name' => _x('EverydayMoney Forms', 'everydaymoney_form'),
            );

            $args = array(
                'labels' => $labels,
                'hierarchical' => true,
                'description' => 'EverydayMoney Forms filterable by genre',
            ...
            register_post_type('everydaymoney_form', $args);
        }
        add_filter('user_can_richedit', 'em_application_tech_disable_wyswyg');

        function em_application_tech_add_view_payments($actions, $post)
        {
            if (get_post_type() === 'everydaymoney_form') {
                unset($actions['view']);
        ...
        add_filter('page_row_actions', 'em_application_tech_add_view_payments', 10, 2);


        function em_application_tech_remove_fullscreen($qtInit)
        {
            $qtInit['buttons'] = 'fullscreen';
            return $qtInit;
        }
        function em_application_tech_disable_wyswyg($default)
        {
            global $post_type, $_wp_theme_features;


            if ($post_type == 'everydaymoney_form') {
                echo "<style>#edit-slug-box,#message p > a{display:none;}</style>";
                add_action("admin_print_footer_scripts", "em_application_tech_shortcode_button_script");
                add_filter('user_can_richedit', '__return_false', 50);
                add_action('wp_dashboard_setup', 'em_application_tech_remove_dashboard_widgets');
                remove_action('media_buttons', 'media_buttons');
                remove_meta_box('postimagediv', 'post', 'side');
                add_filter('quicktags_settings', 'em_application_tech_remove_fullscreen');
            }

            return $default;
        }
        function em_application_tech_remove_dashboard_widgets()
        {
        ...
        add_filter('manage_edit-paystack_form_columns', 'em_application_tech_edit_dashboard_header_columns');

        function em_application_tech_edit_dashboard_header_columns($columns)
        {
            $columns = array(
        ...
        add_action('manage_paystack_form_posts_custom_column', 'em_application_tech_dashboard_table_data', 10, 2);

        function em_application_tech_dashboard_table_data($column, $post_id)
        {
            global $post, $wpdb;
            ...
            switch ($column) {
                case 'shortcode':
					...
					<input type="text" class="large-text code" value="[everydaymoney-form id=&quot;' . $post_id . '&quot;]"
        ...
        add_filter('default_content', 'em_application_tech_editor_content', 10, 2);

        function em_application_tech_editor_content($content, $post)
        {
            switch ($post->post_type) {
                case 'everydaymoney_form':
        ...
        /////
        function em_application_tech_editor_help_metabox($post)
        {
            do_meta_boxes(null, 'custom-metabox-holder', $post);
        }
        add_action('edit_form_after_title', 'em_application_tech_editor_help_metabox');

        function em_application_tech_editor_help_metabox_details($post)
        {
        ...
        function em_application_tech_editor_shortcode_details($post)
        {
        ?>
            <p class="description">
                ...
                <span class="shortcode wp-ui-highlight">
                    <input type="text" id="wpcf7-shortcode" onfocus="this.select();" readonly="readonly" class="large-text code" value="[everydaymoney-form id=&quot;<?php echo $post->ID; ?>&quot;]"></span>
            </p>

        <?php
        }

        add_action('add_meta_boxes', 'em_application_tech_editor_add_extra_metaboxes');
        function em_application_tech_editor_add_extra_metaboxes()
        {
            if ($_GET['action'] == 'edit') {
                add_meta_box('em_application_tech_editor_help_shortcode', 'Paste shortcode on preferred page', 'em_application_tech_editor_shortcode_details', 'everydaymoney_form', 'custom-metabox-holder');
            }
            add_meta_box('em_application_tech_editor_help_data', 'Help Section', 'em_application_tech_editor_help_metabox_details', 'everydaymoney_form', 'custom-metabox-holder');
            add_meta_box('em_application_tech_editor_add_form_data', 'Extra Form Description', 'em_application_tech_editor_add_form_data', 'everydaymoney_form', 'normal', 'default');
            // add_meta_box('em_application_tech_editor_add_recur_data', 'Recurring Payment', 'em_application_tech_editor_add_recur_data', 'everydaymoney_form', 'side', 'default');
            add_meta_box('em_application_tech_editor_add_email_data', 'Email Receipt Settings', 'em_application_tech_editor_add_email_data', 'everydaymoney_form', 'normal', 'default');
            add_meta_box('em_application_tech_editor_add_quantity_data', 'Quantity Payment', 'em_application_tech_editor_add_quantity_data', 'everydaymoney_form', 'side', 'default');
        ...
        function em_application_tech_editor_add_form_data()
        {
            global $post;
            ...
            echo '<select class="form-control" name="_currency" style="width:100%;">
						<option value="NGN" ' . em_application_tech_txncheck('NGN', $currency) . '>Nigerian Naira</option>
						<option value="GHS" ' . em_application_tech_txncheck('GHS', $currency) . '>Ghanaian Cedis</option>
						<option value="ZAR" ' . em_application_tech_txncheck('ZAR', $currency) . '>South African Rand</option>
						<option value="KES" ' . em_application_tech_txncheck('KES', $currency) . '>Kenyan Shillings</option>
						<option value="USD" ' . em_application_tech_txncheck('USD', $currency) . '>US Dollars</option>
				  </select>';
            ...
            echo '<input type="number" name="_amount" value="' . $amount  . '" class="widefat emf-number" />';
            if ($minimum == 1) {
            ...
            echo '<select class="form-control" name="_txncharge" id="parent_id" style="width:100%;">
								<option value="merchant"' . em_application_tech_txncheck('merchant', $txncharge) . '>No, do not add</option>
								<option value="customer" ' . em_application_tech_txncheck('customer', $txncharge) . '>Yes, add it</option>
							</select>
                        <br><small>This allows you include an extra charge to cushion the effect of the transaction fee. <a href="';
            echo get_admin_url() . "edit.php?post_type=everydaymoney_form&page=class-everydaymoney-forms-admin.php#paystack_setting_fees";
            ...
            echo '<select class="form-control" name="_loggedin" id="parent_id" style="width:100%;">
								<option value="no" ' . em_application_tech_txncheck('no', $loggedin) . '>User must not be logged in</option>
								<option value="yes"' . em_application_tech_txncheck('yes', $loggedin) . '>User must be logged In</option>
							</select>';
            ...
            echo '<input ttype="number" name="_filelimit" value="' . $filelimit  . '" class="widefat  emf-number" />';
        ...
        function em_application_tech_editor_add_email_data()
        {
            ...
            echo '<select class="form-control" name="_sendinvoice" id="parent_id" style="width:100%;">
			       <option value="no" ' . em_application_tech_txncheck('no', $sendinvoice) . '>Don\'t send</option>
			       <option value="yes" ' . em_application_tech_txncheck('yes', $sendinvoice) . '>Send</option>
			   </select>';
            echo '<p>Send Email Receipt:</p>';
            echo '<select class="form-control" name="_sendreceipt" id="parent_id" style="width:100%;">
							<option value="no" ' . em_application_tech_txncheck('no', $sendreceipt) . '>Don\'t send</option>
							<option value="yes" ' . em_application_tech_txncheck('yes', $sendreceipt) . '>Send</option>
						</select>';
        ...
        function em_application_tech_editor_add_recur_data()
        {
            ...
            echo '<select class="form-control" name="_recur" style="width:100%;">
							<option value="no" ' . em_application_tech_txncheck('no', $recur) . '>None</option>
							<option value="optional" ' . em_application_tech_txncheck('optional', $recur) . '>Optional Recurring</option>
							<option value="plan" ' . em_application_tech_txncheck('plan', $recur) . '>Paystack Plan</option>
						</select>';
        ...
        function em_application_tech_reset_stock()
        {
        }
        function em_application_tech_editor_add_quantity_data()
        {
            global $post;
            ...
            if ($recur != "no") {
				...
				<option value="no" ' . em_application_tech_txncheck('no', $usequantity) . '>No</option>
			    </select>';
            } else {
                echo '<select class="form-control" name="_usequantity" style="width:100%;">
				<option value="no" ' . em_application_tech_txncheck('no', $usequantity) . '>No</option>
				<option value="yes" ' . em_application_tech_txncheck('yes', $usequantity) . '>Yes</option>
			    </select>';
            }
            if ($usequantity == "yes") {

                echo '<p>Max payable quantity:</p>';
                echo '<input type="number" min="1"  name="_quantity" value="' . $quantity  . '" class="widefat  emf-number" />
				...
				<option value="no" ' . em_application_tech_txncheck('no', $useinventory) . '>No</option>
				<option value="yes" ' . em_application_tech_txncheck('yes', $useinventory) . '>Yes</option>
                </select>
            ...
            if ($useinventory == "yes" && $usequantity  == "yes") {
                ...
                echo '<input type="number" min="' . $sold . '" name="_inventory" value="' . $inventory  . '" class="widefat  emf-number" />';
                echo '<p>Already sold</p>';
                echo '<input type="number" name="_sold" value="' . $sold  . '" class="widefat  emf-number" />
                <small></small>
				<br/>';
            }
        }

        function em_application_tech_editor_add_agreement_data()
        {
            global $post;
            ...
            echo '<select class="form-control" name="_useagreement" style="width:100%;">
					<option value="no" ' . em_application_tech_txncheck('no', $useagreement) . '>No</option>
					<option value="yes" ' . em_application_tech_txncheck('yes', $useagreement) . '>Yes</option>
			</select>';
            echo '<p>Agreement Page Link:</p>';
            echo '<input type="text" name="_agreementlink" value="' . $agreementlink  . '" class="widefat" />';
        }
        function em_application_tech_editor_add_subaccount_data()
        {
            global $post;
            ...
            echo '<select class="form-control" name="_txnbearer" id="parent_id" style="width:100%;">
				<option value="account" ' . em_application_tech_txncheck('account', $txnbearer) . '>Merchant (default)</option>
				<option value="subaccount" ' . em_application_tech_txncheck('subaccount', $txnbearer) . '>Sub Account</option>
			</select>';
            echo '<p>Merchant Amount:</p>';
            echo '<input type="text" name="_merchantamount" value="' . $merchantamount . '" class="widefat" />';
        }
        function em_application_tech_editor_add_startdateplan_data()
        {
            global $post;
            ...
            echo '<input type="number" name="_startdate_days" value="' . $days  . '" class="widefat  emf-number" />';
            echo '<p>Plan:</p>';
        ...
        function em_application_tech_save_data($post_id, $post)
        {
        ...
        add_action('save_post', 'em_application_tech_save_data', 1, 2);
    }

    public function enqueue_styles($hook)
    {
        if ($hook != 'toplevel_page_submissions' && $hook != 'paystack_form_page_class-everydaymoney-forms-admin') {
            return;
        }
        wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/everydaymoney-forms-admin.css', array(), $this->version, 'all');
    }
    public function enqueue_scripts()
    {
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/everydaymoney-forms-admin.js', array('jquery'), $this->version, false);
    }
    ...
    {
        $settings_link = array(
            '<a href="' . admin_url('edit.php?post_type=everydaymoney_form&page=class-everydaymoney-forms-admin.php') . '">' . __('Settings', $this->plugin_name) . '</a>',
        );
        return array_merge($settings_link, $links);
    }
}

add_action('admin_menu', 'em_application_tech_register_newpage');
function em_application_tech_register_newpage()
{
    add_menu_page('everydaymoney', 'everydaymoney', 'administrator', 'submissions', 'em_application_tech_payment_submissions');
    remove_menu_page('submissions');
}

function em_application_tech_payment_submissions()
{
    $id = $_GET['form'];
    $obj = get_post($id);
    if ($obj->post_type == 'everydaymoney_form') {
        ...
        $exampleListTable = new em_t_everydaymoney_Payments_List_Table();
            ...
            <div class="welcome-panel-content">
                ...
                ?>
                    ...
                    <form action="<?php echo admin_url('admin-post.php'); ?>" method="post">
                        <input type="hidden" name="action" value="em_T_export_excel">
...
add_action('admin_post_em_T_export_excel', 'em_t_everydaymoney_export_excel');

function em_t_everydaymoney_prep_csv_data($item)
{
    return '"' . str_replace('"', '""', $item) . '"';
}

function em_t_everydaymoney_export_excel()
{
    global $wpdb;
    ...
    if (count($alldbdata) > 0) {
        ...
        $csv_output .= "#,";
        $csv_output .= "Email,";
        $csv_output .= "AmountPaid,";
        $csv_output .= "AmountSettled,";
        ...
        if (array_key_exists("0", $new)) {
            foreach ($new as $key => $item) {
                $csv_output .= em_t_everydaymoney_prep_csv_data($item->display_name) . ",";
            }
        } else {
            if (count($new) > 0) {
                foreach ($new as $key => $item) {
                    $csv_output .= em_t_everydaymoney_prep_csv_data($key) . ",";
                }
            }
        }
        $csv_output .= "\n";
        ...
        foreach ($alldbdata as $key => $dbdata) {
            ...
            if ($dbdata->txn_code_2 != "") {
                $ourRef = $dbdata->txn_code_2;
            } else {
                $ourRef = $dbdata->ourRef;
            }
            $csv_output .= em_t_everydaymoney_prep_csv_data($newkey) . ",";
            $csv_output .= em_t_everydaymoney_prep_csv_data($dbdata->email) . ",";
            $csv_output .= em_t_everydaymoney_prep_csv_data($currency . ' ' . $dbdata->amount) . ",";
            $csv_output .= em_t_everydaymoney_prep_csv_data($dbdata->settledAmount) . ",";
            $csv_output .= em_t_everydaymoney_prep_csv_data(substr($dbdata->paid_at, 0, 10)) . ",";
            $csv_output .= em_t_everydaymoney_prep_csv_data($ourRef) . ",";
            $new = json_decode($dbdata->metadata);
            $text = '';
            if (array_key_exists("0", $new)) {
                foreach ($new as $key => $item) {
                    $csv_output .= em_t_everydaymoney_prep_csv_data($item->value) . ",";
                }
            } else {
                if (count($new) > 0) {
                    foreach ($new as $key => $item) {
                        $csv_output .= em_t_everydaymoney_prep_csv_data($item) . ",";
                    }
                }
            }
            $csv_output .= "\n";
...
class em_t_everydaymoney_Wp_List_Table
{
...
function em_t_everydaymoney_format_data($data)
{
...
class em_t_everydaymoney_Payments_List_Table extends WP_List_Table
{
    public function prepare_items()
    {
        ...
        foreach ($alldbdata as $key => $dbdata) {
            ...
            $data[] = array(
                ...
                'ourRef' => $ourRef,
                'metadata' => em_t_everydaymoney_format_data($dbdata->metadata),
